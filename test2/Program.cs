...
{   double polyperimetr = 0;
    ...
    for (int i = 0; i < triangles.Length - 2; i++)
    {
        switch(triangles[i])
        {
            case 1 when ((Math.Pow(triangles[0],2) + Math.Pow(triangles[1],2) == Math.Pow(triangles[2],2)) && (triangles[0] < triangles[2]) && (triangles[2]> triangles[1])):
            ...
            break;
            case 2 when ((Math.Pow(triangles[0],2) + Math.Pow(triangles[2],2) == Math.Pow(triangles[1],2)) && (triangles[0] < triangles[1]) && (triangles[1]> triangles[2])):
            Console.WriteLine("Данный треугольник является прямоугольным.");
            ...
            Console.WriteLine($"cos(a): {0:#.##}{area[3]}");
            break;
            case 3 when ((Math.Pow(triangles[1],2) + Math.Pow(triangles[2],2) == Math.Pow(triangles[0],2)) && (triangles[1] < triangles[0]) &&  (triangles[0]> triangles[2])):
            Console.WriteLine("Данный треугольник является прямоугольным.");
            ...
            Console.WriteLine($"cos(a): {0:#.##}{area[3]}");
            break;
            case 4 when (triangles[0] == triangles[1] && triangles[0] != triangles[2]):
            ...
            break;
            case 5 when (triangles[0] == triangles[2] && triangles[0] != triangles[1]):
            Console.WriteLine("Данный треугольник является равнобедернным");
            ...
            break;
            case 6 when (triangles[1] == triangles[2] && triangles[1] != triangles[0]):
            Console.WriteLine("Данный треугольник является равнобедернным");
            ...
            break;
            case 7 when (triangles[0] == triangles[1] && triangles[0] == triangles[2] && triangles[1] == triangles[2]):
